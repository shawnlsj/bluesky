...
public class MySelenium {
    ...
    static Row row;
    static Cell cell;
    static LocalDate limitDate = LocalDate.parse("2010-01-01");
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < liList.size(); i++) {
            ...
            try {
                Info info = tableProcess(tableList);
    ...
    private static void writeToMemory(Info info) {
        //공고날짜(년,월,일)	공고기관	배정예산	수량	공고명	계약방법	예가방법
    ...
    private static Info tableProcess(List<WebElement> tableList) throws Exception {
        Info info = new Info();
        //테이블별로 순회
        for (WebElement table : tableList) {
            ...
            //    info.save("단위", tdList.get(1).getText());
                System.out.println("수량" + tdList.get(0).getText());
                System.out.println("단위" + tdList.get(1).getText());
                continue;
            }
            ...
            for (WebElement tr : trList) {
                ...
                for (WebElement trChildren : trChildrenList) {
                    ...
                    try {
                        ...
                        if (element.getText().equals("공고기관") || element.getText().equals("배정예산")) {
                            key = element.getText();
                            isValid = true;
                        }
                    } catch (Exception e) {
    ...
    static class Info {
        ...
        void save(String key, String value) {
            ...
            } else if (key.equals("게시일시")) {
                //공고날짜가 들어온 경우 LocalDate 로 바꾸고 setValue() 호출
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm");
                LocalDate date = LocalDate.parse(value, formatter);
                map.get(key).setValue(date);
